...
var _extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; }; // es5, 6, and 7 polyfills, powered by babel
...
var parseData = function parseData() {
    ...
    // let data = test_data
    var data = str ? JSON.parse(str) : _data2.default;
...
var info = function info() {
    ...
    return m('.info', { style: { 'background-color': data.colors.toolbar } }, m('h5', { style: { color: data.colors.toolbar_font } }, title));
};
...
var talk = function talk(t) {
    ...
    var style = { 'background-color': data.colors.highlight, color: data.colors.main_font },
        style1 = { 'background-color': data.colors.highlight, color: data.colors.toolbar_font };
    // console.log(t)
    return m('.talk' + c, { style: { 'background-color': data.colors.background } }, t.people && t.people.length ? [m('h1', { style: style }, m('span', t.people[0].name)), m('h3', { style: style1 }, m('span', t.people[0].twitter)), //, ' | ', t.people[0].github)),
    m('h4', { style: style }, m('span', t.title))] : [m('h1', { style: style }, m('span', t.title)), m('h3'), m('h4')], logo()
...
var byTypePriority = function byTypePriority(_ref3, _ref4) {
    var a = _ref3.priority;
    var b = _ref4.priority;
    return a >= b ? -1 : 1;
};

var groupByPriority = function groupByPriority(sponsors) {
    return sponsors.reduce(function (a, v, i) {
        var p = v.priority;
        return _extends({}, a, _defineProperty({}, p, a[p] ? a[p] + 1 : 1));
    }, {});
};
...
var sponsors = function sponsors() {
    ...
    var s = data.sponsors.sort(byTypePriority),
        pgroups = groupByPriority(data.sponsors),
        max = s[0].priority,
        min = s[s.length - 1].priority;

    return m('.sponsors' + (showSponsors ? '.show' : ''), { style: { 'background-color': data.colors.sponsors } }, s.map(function (_ref5) {
        var p = _ref5.priority;
        var i = _ref5.image;
        var size = _ref5.size;
        return m('div', {
            style: {
...
var sub = function sub(t) {
    ...
    return m('.next' + c, { style: { 'background-color': data.colors.toolbar, color: data.colors.toolbar_font } }, m('span', 'Up next: '), m('h6', { style: { color: data.colors.toolbar_font } }, m('span', time(t.start)), t.people && t.people.length ? [' - ', m('span', t.people[0].name)] : [' - ', t.title]));
};

var handleKeypress = function handleKeypress(_ref6) {
...
var logo = function logo() {
    return data.logo ? m('div.logo', { style: { 'background-image': 'url(' + data.logo.url + ')' } }) : '';
};

var APPVIEW = function APPVIEW(m) {
    return function (prev, current, next, fadeOut) {
        return [m('.slides' + (fadeOut ? '.fadeout' : '.fadeout.fadein'), {
        ...
        // m('div', talk(prev)),
        talk(current), sub(next)), sponsors()];
    };
};

var makeCleanable = function makeCleanable(fn) {
    var cleanup = void 0;
    return function () {
        cleanup && cleanup();
        cleanup = fn.apply(undefined, arguments);
    };
};
...
var start = function start(data, view) {
    ...
    }, 1000), data.showSponsors && setInterval(function () {
        showSponsors = true;
        update();
        setTimeout(function () {
            showSponsors = false;
